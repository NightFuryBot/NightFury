 ...
 */
public class UnmuteCmd extends NightFuryCommand {

    /**
     * @param manager
     */
    public UnmuteCmd(NightFuryManager manager) {
        super(manager);
        this.name = "unmute";
    ...
    protected void execute(CommandEvent event) {
        ...
        if(!settings.hasMutedId()) {
            ...
            if(target==null) {
                ...
                executeUnmute(event,settings,target,role,reason);
            }
    ...
    private void executeUnmute(CommandEvent event, Settings settings, Member target, Role role, String reason) {
        String user = FormatUtils.formattedUserName(target.getUser(), true);
        if(!event.getSelfMember().canInteract(target)) {
            event.replyError("I cannot unmute "+user+" because they have a higher role than me!");
        } else if(!event.getSelfMember().canInteract(role)) {
            event.replyError("I cannot unmute "+user+" because the muted role is a higher role than me!");
        } else if(!target.getRoles().contains(role)) {
            event.replyError("I cannot unmute "+user+" because they are not muted!");
        } else {
            event.getGuild().getController().removeRolesFromMember(target, role)
            .reason(reason==null? "No reason specified" : reason).queue(v -> {
                event.replySuccess(user+" was unmuted.");
                if(settings.hasModLogId()) {
                    ...
                    if(!event.getSelfMember().hasPermission(log, Permission.MESSAGE_WRITE))
                        ...
                        LogUtils.userWithReason(settings, log, event.getAuthor(), target.getUser(), LogAction.UNMUTE, reason);
                    else
                        LogUtils.userWithNoReason(settings, log, event.getAuthor(), target.getUser(), LogAction.UNMUTE);
                }
            });
        }
    }
