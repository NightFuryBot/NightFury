...
import xyz.nightfury.entities.starboard.StarMessage
...
object SQLStarboardEntries : Table() {
    ...
    fun addStar(message: Message, user: User) {
        ...
        using(connection.prepareStatement("INSERT INTO STARBOARD_ENTRIES(STARRED_ID, ENTRY_ID, STARBOARD_ID, GUILD_ID, USER_ID) " +
                                          "VALUES (?,?,?,?,?)")) {
            this[1] = starredId
            this[2] = entryId
            this[3] = starboardId
    ...
    fun getStars(message: StarMessage): ArrayList<StarReaction> {
    ...
    fun getStarCount(message: StarMessage): Int {
        var count = 0
    ...
    fun isStarring(message: Message, user: User): Boolean {
        ...
        return using(connection.prepareStatement("SELECT * FROM STARBOARD_ENTRIES WHERE STARRED_ID = ? " +
                                                 "AND USER_ID = ? " +
                                                 "AND GUILD_ID = ?"), false) {
            this[1] = starredId
            this[2] = userId
    ...
    fun setEntry(message: StarMessage) {
        require(message.entryIsCreated) { "An entry was not created" }

        using(connection.prepareStatement("UPDATE STARBOARD_ENTRIES SET ENTRY_ID = ? " +
                                          "WHERE STARRED_ID = ? AND GUILD_ID = ?")) {
    ...
    fun removeEntry(user: User, message: StarMessage) {
    ...
    fun removeAllEntries(message: StarMessage) {
    ...
    private fun findEntryId(starredId: Long, guildId: Long): Long? {
        return using(connection.prepareStatement("SELECT ENTRY_ID " +
