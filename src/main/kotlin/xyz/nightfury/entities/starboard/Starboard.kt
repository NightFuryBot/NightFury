...
import xyz.nightfury.util.db.starboardChannel
import java.time.OffsetDateTime

/**
 * @author Kaidan Gustave
 */
class Starboard(val guild: Guild, val settings: StarboardSettings): MutableMap<Long, StarMessage> by HashMap() {
    ...
    suspend fun addStar(user: User, starred: Message) {
        ...
        if(starred.creationTime.plusHours(settings.maxAge.toLong()).isBefore(OffsetDateTime.now()))
            return
