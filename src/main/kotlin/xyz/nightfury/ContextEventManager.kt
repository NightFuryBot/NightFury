...
class ContextEventManager : IEventManager {
    ...
    override fun handle(event: Event) {
        val job = launch(context, CoroutineStart.LAZY) {
            listeners.forEach { listener ->
                try {
                    (listener as? EventListener)?.apply { onEvent(event) }
                    (listener as? SuspendedListener)?.apply { onEvent(event) }
                } catch(t: Throwable) {
    ...
    private inner class Factory : ThreadFactory {
        private val name get() = "ContextEventManager Thread - ${THREAD_NUMBER.getAndIncrement()}"

        override fun newThread(r: Runnable) = Thread(r, name).also {
            it.isDaemon = true
        }
    }
}
