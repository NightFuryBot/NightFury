...
inline infix fun <reified C: MessageChannel> C.send(init: MessageBuilder.() -> Unit): RestPromise<Message>
    = sendMessage(message { init() }).promise()

inline infix fun MessageBuilder.embed(crossinline init: KEmbedBuilder.() -> Unit): MessageBuilder
...
inline infix fun <reified M: Message> M.edit(block: () -> String): M = apply { editMessage(block()).queue() }
inline infix fun <reified M: Message> M.rebuild(block: MessageBuilder.() -> Unit): M = apply {
