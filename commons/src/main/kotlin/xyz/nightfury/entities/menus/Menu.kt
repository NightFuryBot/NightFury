...
abstract class Menu(builder: Builder<*,*>) {
    ...
    abstract class Builder<B: Builder<B, M>, M: Menu> {
        ...
        inline infix fun waiter(block: () -> EventWaiter): B {
            waiter = block()
            return this as B
        }

        inline infix fun timeout(block: TimeOut.() -> Unit): B {
            TimeOut().apply {
                block()
                timeout = delay
        ...
        inline infix fun role(block: () -> Role): B = plus(block())

        inline infix fun user(block: () -> User): B = plus(block())

        inline infix fun finalAction(crossinline block: (Message) -> Unit): B {
            finalAction = { block(it) }
            return this as B
        }


        inline infix fun displayIn(lazy: () -> MessageChannel) = build().displayIn(lazy())
        inline infix fun displayAs(lazy: () -> Message) = build().displayAs(lazy())
    }
}
