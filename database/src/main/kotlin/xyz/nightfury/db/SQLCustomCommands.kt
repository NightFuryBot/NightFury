...
object SQLCustomCommands : Table() {
    private const val getAll = "SELECT NAME FROM CUSTOM_COMMANDS WHERE GUILD_ID = ?"
    private const val getContent = "SELECT CONTENT FROM CUSTOM_COMMANDS WHERE LOWER(NAME) = LOWER(?) AND GUILD_ID = ?"
    private const val add = "INSERT INTO CUSTOM_COMMANDS (NAME, CONTENT, GUILD_ID) VALUES (?, ?, ?)"
    private const val remove = "DELETE FROM CUSTOM_COMMANDS WHERE LOWER(NAME) = LOWER(?) AND GUILD_ID = ?"
