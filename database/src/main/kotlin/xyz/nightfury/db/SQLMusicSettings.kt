...
import xyz.nightfury.db.entities.MusicSettings
...
object SQLMusicSettings : Table() {
    ...
    private const val select = "SELECT * FROM MUSIC_SETTINGS WHERE GUILD_ID = ?"
    private const val insert = "INSERT INTO MUSIC_SETTINGS(GUILD_ID, VOICE_CHANNEL_ID, TEXT_CHANNEL_ID, NP_IN_TOPIC) VALUES (?,?,?,?)"
    private const val update = "UPDATE MUSIC_SETTINGS SET VOICE_CHANNEL_ID = ?, TEXT_CHANNEL_ID = ?, NP_IN_TOPIC = ? WHERE GUILD_ID = ?"
    ...
    fun getMusicSettings(guild: Guild): MusicSettings? {
        return using(connection.prepareStatement(select)) {
            ...
            using(executeQuery()) {
                if(next()) {
                    MusicSettings(guild, guild.getVoiceChannelById(getLong("VOICE_CHANNEL_ID")), guild.getTextChannelById(getLong("TEXT_CHANNEL_ID")), getBoolean("NP_IN_TOPIC"))
                } else null
            }
        }
    }
