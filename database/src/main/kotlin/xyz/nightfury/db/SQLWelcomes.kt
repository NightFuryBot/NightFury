...
object SQLWelcomes : Table() {
    private const val getMessage = "SELECT MESSAGE FROM WELCOMES WHERE GUILD_ID = ?"
    private const val getChannel = "SELECT CHANNEL_ID FROM WELCOMES WHERE GUILD_ID = ?"
    private const val setWelcome = "INSERT INTO WELCOMES (GUILD_ID, CHANNEL_ID, MESSAGE) VALUES(?,?,?)"
    private const val removeWelcome = "DELETE FROM WELCOMES WHERE GUILD_ID = ?"
    ...
    fun getChannel(guild: Guild): TextChannel? {
        return using(connection.prepareStatement(getChannel)) {
            this[1] = guild.idLong
            using(executeQuery()) {
                if(next()) guild.getTextChannelById(getLong("CHANNEL_ID"))
                else null
            }
        }
    }
