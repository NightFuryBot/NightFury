...
import xyz.nightfury.db.entities.LogAction
 ...
 */
object SQLCases : Table() {
    private const val get = "SELECT * FROM CASES WHERE GUILD_ID = ?"
    private const val getByUser = "SELECT * FROM CASES WHERE GUILD_ID = ? AND MOD_ID = ? ORDER BY NUMBER DESC"
    private const val getNumber = "SELECT * FROM CASES WHERE GUILD_ID = ? AND NUMBER = ?"
    private const val add = "INSERT INTO CASES (NUMBER, GUILD_ID, MESSAGE_ID, MOD_ID, TARGET_ID, IS_ON_USER, ACTION, REASON) " +
                            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    private const val update = "UPDATE CASES SET REASON = ? WHERE GUILD_ID = ? AND NUMBER = ?"
